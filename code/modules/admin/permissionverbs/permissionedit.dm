...
/datum/admins/proc/log_admin_rank_modification(var/adm_ckey, var/new_rank)
	...
	if(!config.sqlite_enabled)
		...
		if(!dbconlite)
			usr << "\red SQLite database is missing"
			return
		...
		var/database/query/q = new("SELECT id FROM erro_admin WHERE ckey = '[adm_ckey]'")
		q.Execute(dbconlite)
		...
		while(q.NextRow())
			new_admin = 0
			admin_id = text2num(q.GetRowData()["id"])

		if(new_admin)
			var/database/query/insertq = new("INSERT INTO `erro_admin` (`id`, `ckey`, `rank`, `level`, `flags`) VALUES (null, '[adm_ckey]', '[new_rank]', -1, 0)")
			insertq.Execute(dbconlite)
			var/database/query/logq = new("INSERT INTO `erro_admin_log` (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Added new admin [adm_ckey] to rank [new_rank]');")
			logq.Execute(dbconlite)
			usr << "\blue New admin added."
		else
			if(!isnull(admin_id) && isnum(admin_id))
				var/database/query/insertq = new("UPDATE `erro_admin` SET rank = '[new_rank]' WHERE id = [admin_id]")
				insertq.Execute(dbconlite)
				var/database/query/logq = new("INSERT INTO `erro_admin_log` (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Edited the rank of [adm_ckey] to [new_rank]');")
				logq.Execute(dbconlite)
				usr << "\blue Admin rank changed."
...
/datum/admins/proc/log_admin_permission_modification(var/adm_ckey, var/new_permission)
	...
	if(!config.sqlite_enabled)
		...
		if(!dbconlite)
			usr << "\red Failed to find SQLite database"
			return
		...
		if(!istext(adm_ckey) || !isnum(new_permission))
			return
		var/database/query/q = new("SELECT id, flags FROM erro_admin WHERE ckey = '[adm_ckey]'")
		q.Execute(dbconlite)
		...
		while(q.NextRow())
			...
			admin_id = text2num(row["id"])
			admin_rights = text2num(row["rank"])

		if(!admin_id)
			return

		if(admin_rights & new_permission) //This admin already has this permission, so we are removing it.
			var/database/query/insertq = new("UPDATE `erro_admin` SET flags = [admin_rights & ~new_permission] WHERE id = [admin_id]")
			insertq.Execute(dbconlite)
			var/database/query/logq = new("INSERT INTO `erro_admin_log` (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Removed permission [rights2text(new_permission)] (flag = [new_permission]) to admin [adm_ckey]');")
			logq.Execute(dbconlite)
			usr << "\blue Permission removed."
		else //This admin doesn't have this permission, so we are adding it.
			var/database/query/insertq = new("UPDATE `erro_admin` SET flags = '[admin_rights | new_permission]' WHERE id = [admin_id]")
			insertq.Execute(dbconlite)
			var/database/query/logq = new("INSERT INTO `erro_admin_log` (`id` ,`datetime` ,`adminckey` ,`adminip` ,`log` ) VALUES (NULL , NOW( ) , '[usr.ckey]', '[usr.client.address]', 'Added permission [rights2text(new_permission)] (flag = [new_permission]) to admin [adm_ckey]')")
			logq.Execute(dbconlite)
			usr << "\blue Permission added."